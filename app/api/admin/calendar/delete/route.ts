...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const qs = new URLSearchParams({name: body.name}).toString();
    const res = await callFrappe(`/api/method/school_os.api.calendar_api.delete_event?${qs}`, {}, sid);
    return NextResponse.json({deleted: (res as any).message?.deleted});
  } catch(e) {
