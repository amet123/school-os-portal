...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const q  = req.nextUrl.searchParams.get('event_type') ?? '';
    const url = `/api/method/school_os.api.calendar_api.get_events${q?`?event_type=${encodeURIComponent(q)}`:''}`;
    const res = await callFrappe(url, {}, sid);
    return NextResponse.json({events: res.message ?? []});
  } catch(e) {
