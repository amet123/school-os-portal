...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const sf = req.nextUrl.searchParams.get('status') ?? '';
    const url = `/api/method/school_os.api.dashboard_api.get_all_certificates${sf?`?status=${encodeURIComponent(sf)}`:''}`;
    const res = await callFrappe(url, {}, sid);
    return NextResponse.json({certificates: (res as any).message ?? []});
  } catch(e) {
