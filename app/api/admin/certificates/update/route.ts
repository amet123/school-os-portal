...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const qs = new URLSearchParams({name: body.name, status: body.status, remarks: body.remarks||''}).toString();
    const res = await callFrappe(`/api/method/school_os.api.dashboard_api.update_certificate_status?${qs}`, {}, sid);
    return NextResponse.json({status: (res as any).message?.status});
  } catch(e) {
