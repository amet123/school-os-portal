...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const {name, status, resolution_notes} = await req.json();
    const res = await callFrappe(
      '/api/method/school_os.api.discipline_api.update_incident_status',
      {method:'POST', body: JSON.stringify({name, status, resolution_notes: resolution_notes||''})},
      sid,
    );
