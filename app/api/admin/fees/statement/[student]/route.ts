...
export async function GET(_req: NextRequest, {params}: {params: {student: string}}) {
  ...
  try {
    ...
    const res = await callFrappe(
      `/api/method/school_os.api.fee_statement_api.get_student_fee_statement?student=${encodeURIComponent(params.student)}`,
      {}, sid,
    );
    return NextResponse.json({statement: res.message ?? {}});
  } catch(e) {
