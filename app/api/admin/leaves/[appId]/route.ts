...
export async function POST(
  ...
  try {
    ...
    if (action === 'approve') {
      endpoint = '/api/method/school_os.api.leave_api.approve_leave';
      body     = {application_id: appId};
    } else if (action === 'reject') {
      endpoint = '/api/method/school_os.api.leave_api.reject_leave';
      body     = {application_id: appId, reason: reason ?? ''};
    } else {
