...
export async function GET(_req: NextRequest, {params}: {params:{student:string}}) {
  ...
  try {
    ...
    const qs = new URLSearchParams({student: params.student}).toString();
    const res = await callFrappe(`/api/method/school_os.api.directory_api.get_student_profile?${qs}`, {}, sid);
