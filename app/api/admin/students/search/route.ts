...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const qs = new URLSearchParams({query: q}).toString();
    const res = await callFrappe(`/api/method/school_os.api.directory_api.search_students?${qs}`, {}, sid);
    return NextResponse.json({students: (res as any).message ?? []});
  } catch(e) {
