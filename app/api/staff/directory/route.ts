...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const res = await callFrappe(`/api/method/school_os.api.directory_api.get_staff_directory${qs.toString()?`?${qs}`:''}`, {}, sid);
    return NextResponse.json({staff: (res as any).message ?? []});
  } catch(e) {
