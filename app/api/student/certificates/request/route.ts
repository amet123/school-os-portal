...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const qs = new URLSearchParams({data: JSON.stringify(body)}).toString();
    const res = await callFrappe(`/api/method/school_os.api.dashboard_api.request_certificate?${qs}`, {}, sid);
    return NextResponse.json({name: (res as any).message?.name});
  } catch(e) {
